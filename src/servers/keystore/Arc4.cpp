...
void
rc4_keysetup(struct rc4_ctx *ctx, uint8 *key, uint32 klen)
{
	uint8 x, y;
	uint32 i;
...
void
rc4_crypt(struct rc4_ctx *ctx, uint8 *src, uint8 *dst,
    uint32 len)
{
	uint32 i;
...
void
rc4_getbytes(struct rc4_ctx *ctx, uint8 *dst, uint32 len)
{
	uint32 i;
...
void
rc4_skip(struct rc4_ctx *ctx, uint32 len)
{
	for (; len > 0; len--) {
		ctx->x = (ctx->x + 1) & (RC4STATE - 1);
		ctx->y = (ctx->state[ctx->x] + ctx->y) & (RC4STATE - 1);
