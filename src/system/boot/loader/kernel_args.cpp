...
{
	uint32 numRanges = gKernelArgs.num_kernel_args_ranges;
	status_t status = insert_address_range(gKernelArgs.kernel_args_range,
		&numRanges, MAX_KERNEL_ARGS_RANGE, (addr_t)start, size);
	gKernelArgs.num_kernel_args_ranges = numRanges;
	return status;
}
...
{
	uint32 numRanges = gKernelArgs.num_physical_memory_ranges;
	status_t status = insert_address_range(gKernelArgs.physical_memory_range,
		&numRanges, MAX_PHYSICAL_MEMORY_RANGE, start, size);
	gKernelArgs.num_physical_memory_ranges = numRanges;
	return status;
}
...
{
	uint32 numRanges = gKernelArgs.num_physical_memory_ranges;
	status_t status = remove_address_range(gKernelArgs.physical_memory_range,
		&numRanges, MAX_PHYSICAL_MEMORY_RANGE, start, size);
	gKernelArgs.num_physical_memory_ranges = numRanges;
	return status;
}
...
{
	uint32 numRanges = gKernelArgs.num_physical_allocated_ranges;
	status_t status = insert_address_range(gKernelArgs.physical_allocated_range,
		&numRanges, MAX_PHYSICAL_ALLOCATED_RANGE, start, size);
	gKernelArgs.num_physical_allocated_ranges = numRanges;
	return status;
}
...
{
	uint32 numRanges = gKernelArgs.num_physical_allocated_ranges;
	status_t status = remove_address_range(gKernelArgs.physical_allocated_range,
		&numRanges, MAX_PHYSICAL_ALLOCATED_RANGE, start, size);
	gKernelArgs.num_physical_allocated_ranges = numRanges;
	return status;
}
...
{
	uint32 numRanges = gKernelArgs.num_virtual_allocated_ranges;
	status_t status = insert_address_range(gKernelArgs.virtual_allocated_range,
		&numRanges, MAX_VIRTUAL_ALLOCATED_RANGE, start, size);
	gKernelArgs.num_virtual_allocated_ranges = numRanges;
	return status;
}
...
{
	uint32 numRanges = gKernelArgs.num_virtual_allocated_ranges;
	status_t status = remove_address_range(gKernelArgs.virtual_allocated_range,
		&numRanges, MAX_VIRTUAL_ALLOCATED_RANGE, start, size);
	gKernelArgs.num_virtual_allocated_ranges = numRanges;
	return status;
}
